...
class Workbook(
  ...
  override def equals(obj: Any): Boolean = obj match {
    case that: Workbook => this.sheetMap == that.sheetMap
    case _ => false
  }

  def copy(sheetMap: Map[String, Sheet] = this.sheetMap, format: WorkbookVersion = this.format): Workbook =
    new Workbook(sheetMap, format)
  ...
  def toStream(stream: OutputStream): Unit =
    book.write(stream)
...
object Workbook {
  ...
  def fromFile(file: File, format: WorkbookVersion = HSSF): Either[Throwable, Workbook] =
    try
      Right(readWorkbook(WorkbookFactory.create(file)))
    catch {
      case e: Throwable => Left(e)
    }

  def fromInputStream(is: InputStream, format: WorkbookVersion = HSSF): Either[Throwable, Workbook] =
    try
      Right(readWorkbook(WorkbookFactory.create(is)))
    catch {
      case e: Throwable => Left(e)
    }

  private def readWorkbook(wb: POIWorkbook): Workbook = {
    val data = for {

...
class AsyncOperationsSpec extends Specification {
  ...
  "AsyncOperations" should {
    ...
    "load workbook asynchronously" in {
      ...
      val originalWorkbook = Workbook(Set(
        Sheet("LoadTest")(Set(
      ...
      originalWorkbook.safeToFile(tempFile.getAbsolutePath).fold(ex => throw ex, identity _).unsafePerformIO()
    ...
    "process data stream asynchronously" in {
      ...
      val futureWorkbook = AsyncOperations.processDataStreamAsync(
        data, transform, "StreamData", batchSize = 25
      )
  ...
  "ReactiveStreams" should {
    ...
    "handle backpressure with memory data source" in {
      ...
      val processor = new AsyncOperations.ReactiveStreams.BackpressureProcessor(
        dataSource, transform, maxBufferSize = 100
      )
  ...
  "JavaInterop" should {
    ...
    "save workbook with Java CompletableFuture" in {
      ...
      val javaFuture = AsyncOperations.JavaInterop.saveWorkbookAsyncJava(
        workbook, tempFile.getAbsolutePath
      )
    ...
    "process data with Java callback" in {
      ...
      val javaFuture = AsyncOperations.JavaInterop.processDataWithCallbackJava(
        javaData, transformer, "JavaCallback", progressCallback
      )
      val workbook = javaFuture.get(5, juc.TimeUnit.SECONDS)
  ...
  "IOOperations" should {
    ...
    "create workbook using Scalaz IO" in {
      val sheets = Set(
        Sheet("IOTest")(Set(
      ...
      val workbook = ioOperation.unsafePerformIO()
      
      workbook.sheets.size mustEqual 1
      workbook.sheets.head.name mustEqual "IOTest"
    }
    
    "save and load workbook using Scalaz IO" in {
      val originalWorkbook = Workbook(Set(
        Sheet("IOSaveLoad")(Set(
      ...
      val loadedWorkbook = saveAndLoadIO.unsafePerformIO()
      
      loadedWorkbook.sheets.size mustEqual 1
      loadedWorkbook.sheets.head.name mustEqual "IOSaveLoad"
    }
    
    "process data stream using Scalaz IO" in {
    ...
    "process multiple sheets using Scalaz IO" in {
      val sheetData = Map(

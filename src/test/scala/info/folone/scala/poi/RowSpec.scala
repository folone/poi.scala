...
class RowSpec extends Specification {
  ...
  "Row styles method" should {
    ...
    "return style map for row with styled cells" in new RowScope {
      val styledCell1 = StyledCell(StringCell(0, "Header"), headerStyle)
      val styledCell2 = StyledCell(NumericCell(1, 100), dataStyle)
    ...
    "combine styles correctly when multiple cells have same style" in new RowScope {
      val styledCell1 = StyledCell(StringCell(0, "A"), headerStyle)
      val styledCell2 = StyledCell(StringCell(2, "C"), headerStyle)

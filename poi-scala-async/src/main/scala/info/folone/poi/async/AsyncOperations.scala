...
object AsyncOperations {
  ...
  class ProgressiveWorkbookBuilder(
    ...
    def addSheetAsync(sheet: Sheet): Future[ProgressiveWorkbookBuilder] =
      Future {
        currentWorkbook = currentWorkbook.copy(sheetMap = currentWorkbook.sheetMap + (sheet.name -> sheet))
  ...
  object JavaInterop {
    ...
    def saveWorkbookAsyncJava(
      ...
      CompletableFuture.runAsync(() => workbook.toFile(path))
